  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow mb-4 dark:bg-gray-900">
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            <button className="bg-gray-100 text-gray-600 px-4 py-2 rounded-lg dark:bg-gray-950 dark:text-white">Filter By</button>
          </div>
          <Popover>
            <PopoverTrigger>
      ...
      <div className="bg-white shadow rounded-lg p-4 dark:bg-black">
        <h2 className="text-lg font-semibold text-gray-700 mb-4 dark:text-white">Recent Orders</h2>
          ...
          <Table className="min-w-full divide-y divide-gray-200">
            <TableHeader className="bg-gray-50 dark:bg-gray-900">
              <TableRow>
                ...
                <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">User Id</TableHead>
                <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Order Date</TableHead>
                ...
                <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Payment Proof</TableHead>
            ...
            <TableBody className="bg-white divide-y divide-gray-200 dark:bg-gray-950">
              {orders && orders.length > 0 && orders.map((order, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell className="px-6 py-4 whitespace-nowrap">{order.products[0].id}</TableCell>
                  ...
                  <TableCell className="px-6 py-4 whitespace-nowrap">{new Date(order.orderDate).toLocaleDateString()}</TableCell>
                  <TableCell className="px-6 capitalize py-4 whitespace-nowrap">{order.paymentMethods}</TableCell>
                  <TableCell className="px-6 capitalize py-4 whitespace-nowrap">{order.paymentProof === '' ? '-' : ( <img src={order.paymentProof} alt="" />)}</TableCell>
                </TableRow>
              ))}
